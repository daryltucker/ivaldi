...
#[test]
fn test_read_file_content_is_raw_text() {
    ...
    // 2. Call read_file on Cargo.toml (known file)
    ...
    // If it's the bug, text will start with '{' (JSON)
    assert!(!text.trim().starts_with('{'), "Text should be raw content, but got JSON: {}", text);
    assert!(text.contains("[package]"), "Text should contain Cargo.toml content");
}
